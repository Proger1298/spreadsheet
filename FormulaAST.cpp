...
public:
    ...
    virtual double Evaluate(const FormulaFunction& formula_function) const = 0;
...
public:
    ...
    double Evaluate(const FormulaFunction& formula_function) const override {
        double result = 0.0;
        switch (type_) {
            case Add:
                result = lhs_->Evaluate(formula_function) + rhs_->Evaluate(formula_function);
                break;
            case Subtract:
                result = lhs_->Evaluate(formula_function) - rhs_->Evaluate(formula_function);
                break;
...
public:
    ...
    double Evaluate(const FormulaFunction& formula_function) const override {
        if (type_ == Type::UnaryMinus) {
            return -1 * operand_->Evaluate(formula_function);
        }
        return operand_->Evaluate(formula_function);
    }
...
double FormulaAST::Execute(const FormulaFunction& formula_function) const {
    return root_expr_->Evaluate(formula_function);
}
