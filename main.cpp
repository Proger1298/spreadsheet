...
void TestCellCircularReferences() {
    ...
    auto sheet1 = CreateSheet();
    sheet1->SetCell("A1"_pos, "=B1");
    sheet1->SetCell("B1"_pos, "=C1");
    sheet1->SetCell("C1"_pos, "=D1");
    sheet1->SetCell("D1"_pos, "Steady");

    bool caught2 = false;
    try {
        sheet1->SetCell("D1"_pos, "=B1+E1");
    } catch (const CircularDependencyException&) {
        caught2 = true;
    }

    ASSERT(caught2);
    ASSERT_EQUAL(sheet1->GetCell("D1"_pos)->GetText(), "Steady");

    auto sheet2 = CreateSheet();
    sheet2->SetCell("A1"_pos, "=B1");

    bool caught3 = false;
    try {
        sheet2->SetCell("B1"_pos, "=A1");
    } catch (const CircularDependencyException&) {
        caught3 = true;
    }

    ASSERT(caught3);
    ASSERT_EQUAL(sheet2->GetCell("B1"_pos)->GetText(), "");
}

void SomeAdditionalTests() {
    {
        auto sheet = CreateSheet();
        sheet->SetCell("A1"_pos, "1");
        sheet->SetCell("A2"_pos, "=A1+3");
        sheet->ClearCell("A1"_pos);
...
int main() {
    ...
    RUN_TEST(tr, SomeAdditionalTests);
}